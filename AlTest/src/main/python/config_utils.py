"""Types used in configuration module for Alchemist's performance tests."""

class Setting(object):
    def __init__(self, name_, val_):
        self.name = name_
        self.val  = val_ 
        
    def to_tuple(self):
        """
        Return tuple of strings each representing a command line option name/value pair.
        """
        return (self.name, self.val.replace(' ', '0x20'))

    def to_array(self):
        """
        Return array of strings each representing a command line option name/value pair.
        """
        return ["--%s=%s" % (self.name, self.val.replace(' ', '0x20'))]
